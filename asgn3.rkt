...
;;status of assignment: passes all test cases
...
(define (parse s)
  (match s
    [(? real? r) (num r)]
    [(list (? symbol? sym) l r) (cond
                                  [(equal? sym '+) (parseBinary s)]
                                  [(equal? sym '-) (parseBinary s)]
                                  [(equal? sym '*) (parseBinary s)]
                                  [(equal? sym '/) (parseBinary s)]
                                  [else (app sym (list (parse l) (parse r)))])]
    ...
    [(list (? symbol? sym)) (cond
                                      [(equal? sym '+) (error "invalid format DXUQ")]
                                      [(equal? sym '-) (error "invalid format DXUQ")]
                                      [(equal? sym '*) (error "invalid format DXUQ")]
                                      [(equal? sym '/) (error "invalid format DXUQ")]
                                      [(equal? sym 'ifleq0) (error "invalid format DXUQ")]
                                      [else (app sym '())])]
    [(list (? symbol? sym) arg ...) (cond
                                      [(equal? sym '+) (error "invalid format DXUQ")]
                                      [(equal? sym '-) (error "invalid format DXUQ")]
                                      [(equal? sym '*) (error "invalid format DXUQ")]
                                      [(equal? sym '/) (error "invalid format DXUQ")]
                                      [(equal? sym 'ifleq0) (error "invalid format DXUQ")]
                                      [else (app sym (parseArgList arg))])]
    [other (error "invalid format DXUQ")]))

(define (parseArgList [s : Sexp]) : (Listof DXUQ2)
  (match s
    [(list a) (cons (parse (first s)) '())]
    [(list a b ...) (cons (parse a) (parseArgList (rest s)))]))

(check-equal? (parseArgList '(1 2 3)) (list (num 1) (num 2) (num 3)))
...
(define (parse-fundef [s : Sexp]) : fundef
  (match s
    ;[(list 'fundef (list (? symbol? sym)) body) (fundef sym '() (parse body))]
    [(list 'fundef (list (? symbol? args) ...) body) (fundef (first (cast args (Listof Symbol)))
                                                             (rest (cast args (Listof Symbol))) (parse (third s)))]
    [other (error "invalid format DXUQ")]))
...
(define (interp-fns funs)
  ...
  (cond
    ...
    [else (interp (fundef-body (first main)) functionList)]))

;check if parameters and arguments are same length and substitute
(define (interp-subst-params [pars : (Listof DXUQ2)]
                             [argsList : (Listof Symbol)] [body : DXUQ2] [funs : (Listof fundef)]) : DXUQ2
  (cond
    [(xor (empty? pars) (empty? argsList)) (error "invalid number of parameters DXUQ")]
    [(empty? pars) body]
    [else (define newBody (subst (num (interp (first pars) funs)) (first argsList) body))
          (interp-subst-params (rest pars) (rest argsList) newBody funs)]))
...
;;subst helper
(define (subst-helper [what : DXUQ2] [for : Symbol] [args : (Listof DXUQ2)]) : (Listof DXUQ2)
  (cond
    [(empty? args) '()]
...
(check-equal? (interp-fns
 ...
 (check-equal? (interp-fns
        (parse-prog '{{fundef {f} 5}
                      {fundef {main} {+ {f} {f}}}}))
       10)
(check-exn (regexp (regexp-quote "invalid number of parameters DXUQ"))
           (lambda () (interp-fns (parse-prog '{{fundef {f x y} {+ x y}}
                             {fundef {main} {f 1}}}))))
